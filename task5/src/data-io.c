...
/*
    ...
    ARGS:
        ...
        int num_of_values               size of x-y data array
*/
int saveData(double* x_data, double* y_data, int num_of_values, const char* output_file_path){
    FILE * output_file;
    output_file = fopen(output_file_path,"w");
    printf("Opening file to write data to: %s\n", output_file_path);
    for (int i = 0; i < num_of_values; ++i)
    {
        int check = fprintf(output_file, DATA_FORMAT"\n", x_data[i], y_data[i]);
        if (check <= 0)
        {
